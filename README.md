...
The job runs on the poll's schedule, daily (`17 11 * * *`). It does not scale
with that frequency the way the poller does: it only looks at listings with no
known price, and remembers the pages that have already said they state none.

Two things about the schedule that cost time to work out. This was `*/2` for a
while, meaning to run every second day — but `*/2` in a day-of-month field does
not mean "every 48 hours", it means the odd days, 1, 3, 5 … 31, so it skipped
every even date and ran twice running across a 31-day month. And GitHub queues
scheduled workflows and runs them late: 11:17 UTC is nominal, the first
scheduled run here fired at 15:07, so read the time as "some time that
morning".
