...
| `.github/workflows/scrape.yml` | Polls the sources every second day and opens a pull request when anything changed |
...
that `*/2` on day-of-month means the 1st, 3rd, 5th … 31st, so a 31-day month is
followed by two runs in a row. That is cron behaving as specified, not a bug,
and two consecutive polls cost the same as any other two.
